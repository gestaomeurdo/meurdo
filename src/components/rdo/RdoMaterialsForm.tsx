  ...
  return (
    <div className="space-y-4">
      ...
      {fields.map((field, index) => {
        ...
        return (
            <div key={field.id} className={cn("p-4 border rounded-xl space-y-4 bg-secondary/5 relative group transition-all", fieldErrors ? "border-destructive/50 bg-destructive/5" : "hover:border-primary/50")}>
            <Button
                type="button"
                variant="ghost"
                size="icon"
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                ...
                <Label className={cn("text-xs uppercase", fieldErrors?.nome_material ? "text-destructive" : "text-muted-foreground")}>Nome do Material</Label>
                <Input
                    placeholder="Ex: Cimento CP V"
                    {...control.register(`materiais.${index}.nome_material`)}
                    className={cn("bg-background", fieldErrors?.nome_material && "border-destructive")}
                />
                </div>
                <div className="md:col-span-3">
                <Label className={cn("text-xs uppercase", fieldErrors?.unidade ? "text-destructive" : "text-muted-foreground")}>Unidade</Label>
                <Input
                    placeholder="Ex: Saco, m3, un"
                    {...control.register(`materiais.${index}.unidade`)}
                    className={cn("bg-background", fieldErrors?.unidade && "border-destructive")}
                />
                </div>
