  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center px-1">
        ...
        <Badge variant="outline" className="text-[10px] font-black border-primary/20 text-primary dark:text-blue-400 dark:bg-primary/5 uppercase px-3">
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        {[
          { label: "Aprovação Pendente", value: stats.pending, icon: FileClock, color: "bg-orange-500", text: "text-orange-600 dark:text-orange-400", bg: "bg-orange-500/10", border: "border-orange-500/20" },
          { label: "RDOs Aprovados", value: stats.approved, icon: FileCheck, color: "bg-emerald-500", text: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-500/10", border: "border-emerald-500/20" },
          { label: "Em Rascunho", value: stats.draft, icon: FileEdit, color: "bg-slate-400", text: "text-slate-500 dark:text-slate-400", bg: "bg-slate-400/10", border: "border-slate-400/20" },
          { label: "Média de Efetivo", value: stats.avgManpower, icon: Users, color: "bg-blue-500", text: "text-blue-600 dark:text-blue-400", bg: "bg-blue-500/10", border: "border-blue-500/20" },
        ].map((item, i) => (
          <Card key={i} className="rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-clean bg-card overflow-hidden group">
              ...
              <CardContent className="p-6">
                  <div className="flex justify-between items-start mb-4">
                      <div className={cn("p-3 rounded-2xl transition-transform group-hover:scale-110 border", item.bg, item.border)}>
