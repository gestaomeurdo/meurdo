...
const statusConfig: Record<TicketStatus, { label: string, color: string, icon: any }> = {
  ...
  'resolved': { label: 'Respondido', color: 'bg-emerald-500', icon: CheckCircle2 },
    ...
    <div className="rounded-2xl border overflow-hidden bg-card shadow-sm">
      <Table>
        <TableHeader className="bg-muted/50">
          <TableRow>
            <TableHead className="text-[10px] font-black uppercase tracking-widest pl-6">Assunto</TableHead>
            <TableHead className="text-[10px] font-black uppercase tracking-widest">Status</TableHead>
            <TableHead className="text-[10px] font-black uppercase tracking-widest text-right pr-6">Data</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {tickets.map((ticket) => {
                  ...
                  <TableRow className="cursor-pointer hover:bg-accent/50 transition-colors group">
                    <TableCell className="pl-6">
                      <div className="flex flex-col">
                        ...
                        <span className="text-[9px] text-muted-foreground font-black uppercase tracking-tighter">{ticket.category}</span>
                      </div>
                    </TableCell>
                    <TableCell>
