...
import { AlertCircle, Clock, ChevronRight, FileClock, AlertTriangle } from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 px-1">
        <h2 className="text-xl font-black uppercase tracking-tight text-slate-800">Ações Pendentes</h2>
        <Badge variant="secondary" className="bg-orange-100 text-orange-600 border-none font-black">{rdos.length}</Badge>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {rdos.map((rdo) => (
          <Link key={rdo.id} to={`/obras/${rdo.obra_id}`}>
            <Card className={cn(
                "border-none shadow-sm hover:shadow-md transition-all rounded-[1.5rem] overflow-hidden group",
                rdo.status === 'rejected' ? "bg-red-50/50 ring-1 ring-red-100" : "bg-white ring-1 ring-slate-100"
            )}>
              <CardContent className="p-5 flex items-center gap-4">
                <div className={cn(
                    "w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 transition-transform group-hover:scale-110",
                    rdo.status === 'rejected' ? "bg-red-100 text-red-600" : "bg-orange-100 text-orange-600"
                )}>
