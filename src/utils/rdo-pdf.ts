...
export const generateRdoPdf = async (
  ...
  try {
    console.log("[PDF Generator] Preparando ativos...");

    // 1. Tratamento Robusto de Data (Pode vir como String ou Date)
    let dateStr = "";
    ...
    // 2. Cálculo do Número Sequencial
    ...
    // 3. Pre-fetching (Logo, Fotos, Assinaturas)
    ...
    // 4. Renderização (Passando a data já tratada como string)
    ...
    console.log("[PDF Generator] Sucesso.");
  } catch (error) {
    console.error("[PDF Generator] Erro Crítico:", error);
    throw new Error("Falha técnica ao processar o arquivo PDF. Tente salvar o RDO antes de exportar.");
  }
};