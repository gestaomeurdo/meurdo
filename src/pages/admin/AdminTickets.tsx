...
import { Loader2, Send, User, MessageSquare, ShieldCheck, Mail, Search } from "lucide-react";
...
const AdminTickets = () => {
  ...
  const [selectedTicketId, setSelectedTicketId] = useState<string | null>(location.state?.ticketId || null);
  const [replyText, setReplyText] = useState("");
  const [search, setSearch] = useState("");
  
  const { data: conversations, isLoading: loadingInbox } = useAdminInbox();
  const { data: messages, isLoading: loadingMessages } = useAdminChatMessages(selectedTicketId || undefined);
  ...
  const filteredConversations = conversations?.filter(c => 
    (c as any).email?.toLowerCase().includes(search.toLowerCase()) ||
    (c as any).first_name?.toLowerCase().includes(search.toLowerCase())
  );

  const selectedRoom = conversations?.find((r: any) => r.ticket_id === selectedTicketId);
      ...
      <div className="flex h-[calc(100vh-4px)] bg-slate-950 overflow-hidden">
        ...
        <div className="w-80 sm:w-[380px] border-r border-slate-800 flex flex-col bg-slate-900/40">
            ...
            <ScrollArea className="flex-1">
                ...
                ) : filteredConversations?.map((room: any) => (
                    <div 
                        key={room.ticket_id} 
                        onClick={() => setSelectedTicketId(room.ticket_id)}
                        className={cn(
                            "p-5 border-b border-slate-800/50 cursor-pointer transition-all hover:bg-white/5 relative group",
                            selectedTicketId === room.ticket_id ? "bg-blue-600/10 border-l-4 border-l-blue-600" : ""
                        )}
                    >
                        <div className="flex justify-between items-start mb-1">
                            <span className="text-[9px] font-black uppercase text-blue-400 tracking-widest">{room.plan_type || 'FREE'}</span>
                            <span className="text-[9px] font-bold text-slate-500">{format(parseISO(room.last_message_at), "dd/MM")}</span>
                        </div>
                        <h3 className="text-sm font-bold text-slate-100 truncate">{room.first_name} {room.last_name}</h3>
                        <p className="text-[10px] text-slate-500 mt-1 truncate">{room.email}</p>
                        {room.status === 'open' && (
                ...
                <>
                    <header className="p-6 border-b border-slate-800 bg-slate-900/30 flex justify-between items-center backdrop-blur-md">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-lg font-black uppercase tracking-tight text-white leading-none mb-1">{(selectedRoom as any).first_name} {(selectedRoom as any).last_name}</h2>
                                <p className="text-[10px] font-bold uppercase text-slate-500 tracking-widest">{(selectedRoom as any).email}</p>
                            </div>
                        </div>
                    </header>
