...
const Materiais = () => {
  ...
  const { data: receipts, isLoading: isLoadingReceipts, isFetching: isFetchingReceipts, error: receiptsError, refetch } = useMaterialReceipts(selectedObraId);
      ...
      return (
        <Alert variant="destructive" className="mt-6 border-2">
          ...
          <AlertDescription className="flex flex-col gap-4 mt-2">
            <p className="text-sm">
              O sistema não conseguiu conectar com o banco de dados de materiais. Isso pode ser um problema de permissão ou conexão.
            </p>
        ...
        <Card className="border-dashed py-20 text-center shadow-none bg-muted/20 rounded-3xl">
          <CardContent>
            ...
            <Button asChild className="rounded-xl">
              <a href="/obras">Cadastrar Primeira Obra</a>
            </Button>
          </CardContent>
        </Card>
      );
      ...
      return (
        <div className="text-center py-20 border border-dashed rounded-3xl bg-muted/10">
          ...
          <p className="text-muted-foreground font-medium">
            Selecione uma obra no menu acima para gerenciar os materiais.
          </p>
        </div>
      );
    }
      ...
      <div className="space-y-6">
        <Alert className="bg-primary/5 border-primary/20 rounded-2xl">
          ...
          <AlertDescription className="text-xs">
            Materiais registrados aqui são importados automaticamente para o <strong>RDO</strong> da respectiva data.
          </AlertDescription>
        </Alert>
        ...
        {(isLoadingReceipts || isFetchingReceipts) && !receipts ? (
          ...
          <div className="text-center py-24 bg-muted/5 rounded-3xl border-2 border-dashed border-muted-foreground/20">
            ...
            <p className="text-sm text-muted-foreground max-w-xs mx-auto mt-1">
              {search
                ? "Não encontramos nada com esse termo de busca."
                : "Comece registrando o primeiro recebimento de material para esta obra."}
            </p>
            {search && (
              <Button
                variant="link"
          ...
          <div className="grid grid-cols-1 gap-4">
            {filteredReceipts?.map((receipt) => (
              <Card
                key={receipt.id}
                className="overflow-hidden hover:shadow-md transition-all border-l-4 border-l-primary shadow-clean rounded-2xl group"
              >
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div className="flex items-start gap-4">
                      <div className="w-16 h-16 rounded-xl bg-accent/50 flex items-center justify-center shrink-0 overflow-hidden border border-primary/10 shadow-inner group-hover:border-primary/30 transition-colors">
                        {receipt.foto_url ? (
                          <a
                            href={receipt.foto_url}
                            target="_blank"
                            rel="noreferrer"
                            className="w-full h-full"
                          >
                            <img
                              src={receipt.foto_url}
                              alt="NF"
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center flex-wrap gap-2">
                          ...
                          <Badge
                            className={cn(
                              "text-[10px] font-black px-2 rounded-full uppercase tracking-wider",
                              receipt.status === 'Conforme'
                        ...
                        <p className="text-sm text-primary font-black uppercase tracking-widest">
                          {receipt.quantidade} {receipt.unidade}
                        </p>
                        <div className="flex flex-wrap gap-x-4 gap-y-2 text-[11px] text-muted-foreground pt-1">
                          <span className="flex items-center gap-1.5 font-bold uppercase">
                            <Truck className="w-3.5 h-3.5 text-primary" />
                            {receipt.fornecedor || 'N/A'}
                          </span>
                          <span className="flex items-center gap-1.5 font-bold uppercase">
                            <Calendar className="w-3.5 h-3.5 text-primary" />
                            {format(new Date(receipt.data_recebimento), 'dd/MM/yyyy', {
                              locale: ptBR,
                            })}
                          </span>
                          {receipt.numero_nf && (
                            <span className="flex items-center gap-1.5 font-mono font-black uppercase bg-primary/10 text-primary px-2 py-0.5 rounded">
                              NF: {receipt.numero_nf}
                            </span>
                          )}
                        </div>
                      </div>
                        ...
                        <AlertDialogContent className="rounded-3xl border-2">
                          ...
                          <AlertDialogFooter className="gap-2">
                            ...
                            <AlertDialogAction
                              ...
                              className="bg-destructive rounded-xl hover:bg-destructive/90 font-bold uppercase tracking-widest text-xs"
                            >
                              Excluir Permanente
                            </AlertDialogAction>
                          </AlertDialogFooter>
      ...
      <div className="p-4 sm:p-6 space-y-6 animate-in fade-in duration-700">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-sm text-muted-foreground font-medium">
            Controle de recebimentos, qualidade e histórico de entregas.
          </p>
        </div>
