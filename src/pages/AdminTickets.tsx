...
import { Loader2, Send, User, LifeBuoy, CheckCircle2, AlertCircle, Clock, ExternalLink, ShieldCheck, Mail, Construction } from "lucide-react";
...
const AdminTickets = () => {
  ...
  const isAdmin = user?.email === 'robsonalixandree@gmail.com';
  ...
  return (
    <DashboardLayout>
      <div className="flex flex-col h-[calc(100vh-64px)]">
        ...
        <div className="bg-purple-600 text-white px-6 py-1.5 flex items-center justify-between shadow-lg z-10">
            <div className="flex items-center gap-2">
                <ShieldCheck className="w-3.5 h-3.5" />
                <span className="text-[10px] font-black uppercase tracking-[0.2em]">Modo Administrador Ativo • Robson</span>
            </div>
            <div className="text-[9px] font-bold opacity-80 uppercase tracking-widest">Acesso de Manutenção do Sistema</div>
        </div>
                ...
                <>
                    ...
                    <header className="p-6 border-b bg-slate-50/50 dark:bg-slate-900/30 flex justify-between items-center">
                        ...
                        <div className="flex gap-2">
                            <Button variant="outline" size="sm" className="rounded-xl h-10 font-bold text-xs" asChild>
                                <Link to="/obras">
                                    <Construction className="w-4 h-4 mr-2" /> Investigar Obras
                                </Link>
                            </Button>
                            <Button variant="secondary" size="sm" className="rounded-xl h-10 font-bold text-xs" onClick={handleMarkResolved}>
